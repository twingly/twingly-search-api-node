...
Parser.prototype.parse = function(document, callback){
    ...
    if(xpath.select("//*[local-name(.)='operationResult' and namespace-uri(.)='http://www.twingly.com'][@resultType='failure']", doc).length > 0) {
        callback(this._handleFailure(xpath.select("//*[local-name(.)='operationResult' and namespace-uri(.)='http://www.twingly.com'][@resultType='failure']/text()", doc)[0].toString()), null);
    } else if(xpath.select("//twinglydata", doc).length == 0) {
        callback(this._handleNonXmlDocument(doc), null);
    } else {
        callback(false, this._createResult(doc));
    }
};
Parser.prototype._createResult = function(data_node){
    ...
    for(var i = 0; i < posts.length; i++) {
        result.posts.push(this._parsePost(posts[i]));
    }

    return result;
};

Parser.prototype._parsePost = function(element){
    ...
    for(var i = 0; i < element.childNodes.length; i++) {
        if((element.childNodes[i].tagName != '')&&(element.childNodes[i].tagName != undefined)) {
            if(element.childNodes[i].tagName == 'tags') {
                post_params[element.childNodes[i].tagName] = this._parseTags(element.childNodes[i]);
            } else {
...
Parser.prototype._parseTags = function(element){
    var tags = [];
...
Parser.prototype._handleFailure = function(failure){
    return (new TwinglyError()).fromApiResponseMessage(failure);
};

Parser.prototype._handleNonXmlDocument = function(document){
